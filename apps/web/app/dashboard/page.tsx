...
import { Terminal, Copy, ArrowRight, Play, BookOpen, Check, ArrowUpRight } from "lucide-react";
        ...
        <div className="md:w-2/3 bg-surface border border-border rounded-2xl p-6 relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-6 min-h-[300px]">
            ...
            {activeEditor === "Cursor" && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <div>
                  <p className="text-sm font-medium text-main mb-3">1. Add to Rules for AI</p>
                  <div className="bg-bg border border-border rounded-lg p-3 flex justify-between items-center group">
                    <code className="text-sm font-mono text-muted">Use Grafz MCP: https://grafz.io/mcp</code>
                    <button onClick={() => handleCopy("Use Grafz MCP: https://grafz.io/mcp", "step1")} className="text-faint hover:text-main">
                      {copied === "step1" ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" />}
                    </button>
                  </div>
                  <p className="text-sm text-faint mt-3">Go to Cursor Settings {'>'} Rules for AI and paste the MCP server link.</p>
                </div>
              </motion.div>
            )}
            ...
            {activeEditor === "Codex" && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <div>
                  <p className="text-sm font-medium text-main mb-3">1. Install Codex Extension</p>
                  <div className="bg-bg border border-border rounded-lg p-3 flex justify-between items-center group">
                    <code className="text-sm font-mono text-muted">codex ext install grafz-memory</code>
                    <button onClick={() => handleCopy("codex ext install grafz-memory", "step1")} className="text-faint hover:text-main">
                      {copied === "step1" ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" />}
                    </button>
                  </div>
                </div>
            ...
            {(activeEditor !== "Claude Code" && activeEditor !== "Cursor" && activeEditor !== "Codex") && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <div>
                  <p className="text-sm font-medium text-main mb-3">1. Generic NPM Install</p>
                  <div className="bg-bg border border-border rounded-lg p-3 flex justify-between items-center group">
                    <code className="text-sm font-mono text-muted">npm install -g @grafz/{activeEditor.toLowerCase().replace(" ", "-")}</code>
                    <button onClick={() => handleCopy(`npm install -g @grafz/${activeEditor.toLowerCase().replace(" ", "-")}`, "step1")} className="text-faint hover:text-main">
                      {copied === "step1" ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" />}
                    </button>
                  </div>
                  <p className="text-sm text-faint mt-3">Authenticate via the CLI after installation.</p>
                </div>
              </motion.div>
            )}
            ...
            <div className="pt-6 border-t border-border border-dashed">
              ...
              <a href="#" className="inline-block mt-4 text-sm flex items-center gap-1.5 text-faint hover:text-main transition-colors group">
                {activeEditor} docs 
